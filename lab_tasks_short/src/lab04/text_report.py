...
def freq(text):
    return Counter(tokenize(normalize(text)))


def sort_freq(f):
    return sorted(f.items(), key=lambda x: (-x[1], x[0]))
...
def main():
    ...
    try:
        if len(p.files) == 1 and not p.per_file:
            ...
            allf, per = Counter(), []
            for file in p.files:
                f = freq(read_text(file, p.encoding))
                allf += f
                for w, c in sort_freq(f):
                    per.append((Path(file).name, w, c))
            if p.per_file:
                per.sort(key=lambda x: (x[0], -x[2], x[1]))
                write_csv(per, p.per_file, ("file", "word", "count"))
            if p.total:
                write_csv(sort_freq(allf), p.total, ("word", "count"))
